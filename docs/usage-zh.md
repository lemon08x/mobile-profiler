...
1. 如果手机还未连接，在顶部 **ADB IP / HDC IP** 输入 `IP:端口`；鸿蒙地址加
   `harmony:` 前缀，例如 `harmony:192.168.1.20:8710`。
2. 选择处于 `device` 状态的手机。
...
HarmonyOS 手机 <-- Wi-Fi HDC -->+--> Mobile Power Profiler --> 原始数据和报告
      ^
      |
机械臂/相机
      |
...
mobile-power-profiler --hdc D:\DevEco\hdc.exe probe --platform harmony --device harmony:PHONE_IP:8710
```
...
- 当前 HarmonyOS 量产机的 GPU sysfs 对 HDC shell 受限，而且没有 Android
  `dumpsys gpu` 回退；RenderService 仍可提供 Maleoon renderer，但报告不推断频率、
  负载或应用能量。
- HarmonyOS RenderService 的合成器 FPS 是“最近提交记录”的周期抽样，不是应用内部
  渲染线程 FPS；MultimodalInput 触摸次数也不是面板硬件触控采样率。
