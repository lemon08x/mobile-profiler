...
- **目标游戏 / 应用**：性能测试必填。Android 先选择设备，再点击 **扫描手机应用**；
  ...
  包名收在“更多采集设置”中且始终可选，多应用 BTR2 流程可留空并启用 session mode；性能模式会把
  目标游戏 / 应用移到“必要设置”并要求填写，且不提供多应用
...
- **设备亮度（Android）**：选择在线设备后，界面会读取当前整数亮度，并提示该机的
  ...
  防止中途改变测试条件。
- **设备亮度（HarmonyOS）**：通过 `DisplayPowerManagerService` 读取逻辑请求亮度、
  `DeviceBrightness`、显示折扣、自动亮度状态及服务声明的 `1–255` 理论范围，并用同一
  服务回读验证设置结果。若系统镜像提供官方 `ohos-displayManager set-brightness`，工具优先
  直接调用；量产镜像通常未预装该命令，而且服务端的 `SetBrightness` 与
  `AutoAdjustBrightness` 明确要求系统应用身份，普通 HDC shell 或普通签名 HAP helper
  无法绕过。因此设置页兼容通道不会再把理论范围内的任意整数当成可写值：首次点击
  “扫描可选值”后，程序先把滑块移到相反端，再对候选位置落点并等待 DPM 稳定回读，利用
  单调阶梯映射自适应细分出所有真实档位。扫描通常需要约 3–6 分钟，结果按 HDC 设备和
  滑杆布局缓存；完成后亮度编辑器变为下拉框，只列出已验证档位，API 也会拒绝未校准或
  不可达值。扫描和设置结束会恢复原前台游戏与原屏幕状态；原亮度本身不可达时恢复到
  最近且较低的档位并明确提示。目标值已相同且当前为手动亮度时不会打开设置页。界面会
  明确标出“系统直写”或“设置页兼容”，不会把 `hidumper` 的只读能力误报为直接可写。
  本次连接的 nova 实测可选范围为 `1–237`、共 `191` 档；除以下 `46` 个值外，其余
  `1–237` 整数均可选：`83, 93, 100, 106, 111, 116, 121, 125, 129, 133, 137, 141,
  144, 148, 151, 155, 158, 161, 164, 167, 170, 173, 176, 179, 182, 185, 187, 190,
  193, 195, 198, 201, 203, 206, 209, 211, 214, 216, 219, 221, 223, 226, 228, 231,
  233, 235`。例如 `106` 无对应稳定落点，最近的较低可选值是 `105`。
- **设备亮度（iPhone）**：通过 DiagnosticsService 的 `AppleARMBacklight` IORegistry
  同时读取 0–100% 用户亮度、`rawBrightness` 原始背光和 `BrightnessMilliNits` 校准
  毫尼特值。UI 会显示当前百分比、实际 nits 和原始背光，并允许手动刷新验证。iOS 没有
  向当前 sidecar 公开可信的通用外部亮度 setter，也没有公开自动亮度开关，因此 iPhone
  亮度控件明确为只读：请先在手机端调整，再点击“读取亮度”复核。程序不会通过固定坐标
  拖动控制中心滑杆来伪装成精确数值设置。
- **屏幕热降亮监控（Android）**：开启“温度 / 热限制”采集时，程序会把系统设定亮度、
  ...
  不会按降亮处理。
- **屏幕热降亮监控（HarmonyOS）**：把逻辑亮度、`DeviceBrightness`、`Discount`、
  RenderService backlight 与 ThermalService 的外壳/系统温度联合记录。手动亮度设定
  保持不变、显示侧确实发生折扣或设备亮度下降，并同时出现外壳/系统温度证据时标为
  “确认”；只有显示限制而温度证据不足时标为“疑似”。屏幕处于 OFF、DOZE 或 DIM 时不
  作为热降亮，限制恢复后仍保留历史点供实时图和报告查看。
- **屏幕热降亮监控（iPhone）**：开启“温度 / 热限制”后，每 5 秒记录用户亮度、
  `rawBrightness` 和实际毫尼特值。用户亮度保持在同一档位时，实际 nits 与原始背光相对
  本轮同档位基线同步下降会标为“疑似”；若同时出现
  `com.apple.system.thermalpressurelevel` / `earlythermalnotification` 或明显电池温升，
  则提高为“确认”。缺少热证据时不会直接归因于过热，因为自动亮度、OLED 内容限制和
  系统显示算法也可能改变实际毫尼特。实时图和离线报告都会保留逐点时间、nits、原始
  背光、温度、前台应用和判定原因。
- **开始场景**：通常选择“桌面”。
...
- Android / iOS / HarmonyOS 实时图中的全部疑似/确认热降亮点。Android/HarmonyOS
  展示逻辑/系统设定到显示侧有效档位的变化；iOS 展示用户亮度、rawBrightness 和实际
  nits 相对同档位基线的变化。历史点在亮度恢复后仍会保留到本轮测试结束，并进入离线报告。
...
- Android ADB 能确认 Framework / Thermal HAL 已触发的显示限亮，并估算归一化有效亮度；
  ...
  需要外部亮度计才能做精确光学测量。
- HarmonyOS 的 `DisplayPowerManagerService` dump 可以读取请求值、设备值和折扣，但其
  写接口从 OpenHarmony 5.0.3 起即检查 `Permission::IsSystem()`；只读字段完整不代表 HDC
  shell 具备 setter 权限。`Discount` 也不保证只由温控产生，因此必须结合手动亮度稳定性
  与 ThermalService 温度判断；精确物理亮度同样需要外部亮度计。
- iPhone 的 `AppleARMBacklight` 可以提供用户亮度、原始背光和校准毫尼特值，比只看
  系统滑杆更接近显示驱动的实际输出；但它不公开通用 setter 或自动亮度开关。即使 nits
  下降，也必须结合用户亮度稳定性、热压力通知和温升判断，不能把 OLED 内容限制或环境光
  调节直接当成热降亮。毫尼特字段来自设备校准数据，精确光学复核仍应使用外部亮度计。
...
- `AppleARMBacklight` 用户亮度、`rawBrightness`、实际毫尼特与 iOS 热压力通知；实时 UI
  和报告会标记同档位疑似/确认降亮点。
...
- iPhone 亮度来自 `AppleARMBacklight.IODisplayParameters`：`brightness` 转为用户亮度
  百分比，`rawBrightness` 保留驱动原始背光，`BrightnessMilliNits` 转为实际 nits。
  UI 不提供 iPhone 外部写入；请在手机端调整并重新读取。降亮分析只在同一用户亮度下
  比较本轮基线，缺少热压力通知或温升时仅标为“疑似”。
