...
左侧 **AI 自动化** 是当前独立于功耗采集的基础功能块，只支持 Android ADB。它复用
BTR2 已部署的 OpenAI-compatible/vLLM 模型 API，不依赖 BTR2 的 Python 代码、相机或
机械臂。

使用前确认：

- 右上角已经选择一台处于 `device` 状态的 Android 手机。
- 运行 Mobile Profiler 的电脑可以访问 BTR2 模型服务器。
- 当前 BTR2 默认大模型地址为 `http://192.168.31.237:8000`，模型为
  `qwen3.6-27b`；UI 中可以临时修改。
- 也可以在启动 UI 前设置 `BTR2_LLM_ENDPOINT`、`BTR2_LLM_MODEL` 和
  `BTR2_LLM_TOKEN` 作为默认值。

操作步骤：

1. 进入 **AI 自动化**。
2. 输入清晰、可从屏幕判断是否完成的任务，例如：
   “打开系统设置，关闭自动亮度，把亮度调整到约 50%，完成后结束任务”。
3. 核对模型 API、模型名称、最大步骤、动作后等待和单次模型超时。
4. 点击 **启动闭环任务**。
5. 观察右侧最新 ADB 截图、步骤、模型判断、`phone_action` 和执行结果。
6. 模型调用 `finish` 后正常结束；调用 `take_over`、达到步骤上限或发生错误时，
   根据提示人工处理。任何时候都可以点击 **停止任务**。
...
adb exec-out screencap -p
  -> 截图 + 用户任务 + 最近动作结果
  -> BTR2 /v1/chat/completions 原生工具调用
...
```text
profiler-runs\agent-runs\时间-任务-随机后缀\
  config.json
  events.jsonl
  step-001.png
  step-002.png
...
当前版本尚未把 Agent 的开始/结束和动作自动写入功耗测试时间线，也没有内置手机
初始化模板或外部 API 工具。先通过本功能块验证截图、模型决策和 ADB 执行闭环；后续
再把同一控制器接入功耗会话、流程模板和外部服务。

## 8. 与 BTR2 不同步开始时怎么做

对于功耗采集与日志对齐，Mobile Profiler 和 BTR2 仍是两个独立进程；上一节的 AI
自动化只复用 BTR2 模型 API，不改变两边的采集生命周期：
