...
5. 核对模型 API、模型名称、动作后等待和单次模型请求超时。
6. 如需适配特殊 ROM 或测试制度，展开 **ADB Agent System Prompt** 编辑全局规则；
   随时可以点击 **恢复默认** 回到内置版本。
7. 点击 **启动测试流程**。
8. 观察右侧最新 ADB 截图、当前子任务、子任务内步骤、模型判断、`phone_action`、
   逐项结果和闭环日志。
9. 模型调用 `finish` 时只完成当前子任务，编排器会继续下一项；调用 `take_over` 时
