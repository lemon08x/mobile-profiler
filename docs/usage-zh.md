# Mobile Power Profiler 使用指南

本文以 Windows 为主，按实际操作顺序说明如何使用 Mobile Power Profiler。Android
和兼容 ADB 调试通道的 HarmonyOS 设备走标准库主流程；iOS 通过独立的可选 sidecar
运行时接入。三端都可以与 BTR2 配合、保存原始证据并进入同一套报告和对比流程。
...
完整解压 `mobile-power-profiler-portable.zip`，然后双击解压目录中的：
...
以下是 Android/HarmonyOS 的 ADB 标准顺序。iPhone 的首次 RemotePairing 和拔线流程
见第 17 节。

1. 如果手机还未连接，在顶部 **ADB IP** 输入 `IP:端口`，点击 **连接**。
2. 选择处于 `device` 状态的手机。
3. 进入 **设备能力**，执行只读 **Probe**。
4. 确认正式测试时手机没有 USB、AC 或无线充电供电。
5. 回到 **实时监控**，填写运行名称、时长和起始说明，点击 **开始采集**。
6. 正常运行 BTR2；两边不必同时开始。
7. 必要时点击 **添加时间标记**，记录“BTR2 开始”“进入视频测试”等事件。
8. 采集结束后，在 **工具与交付** 中导入 BTR2 日志、重建报告、生成证据 ZIP。
9. 两台手机均完成后，在同一页面生成双机对比报告。
...
- Android 或 HarmonyOS 手机已打开开发者选项和可用的 USB/无线 ADB 调试。
...
1. 确认当前没有手机采集正在运行。
2. 保持默认输出目录
   `dist\mobile-power-profiler-portable`。
...
```text
dist\mobile-power-profiler-portable\
dist\mobile-power-profiler-portable.zip
```
...
- 整机进程、ThermalService、cpuset 和 ADPF 数据源是否可见。
...
- **时长**：一小时 BTR2 测试建议使用 `3720` 秒。先启动功耗采集并等待界面出现首个实时样本，再启动 BTR2；额外两分钟用于覆盖启动准备和两边的时间偏差。
- **采样间隔**：默认 `1` 秒通常足够。
- **目标包名**：单应用测试可填写；多应用 BTR2 流程可留空并启用 session mode。
- **开始场景**：通常选择“桌面”。
...
- 前台包名、Activity、屏幕、亮度和刷新率。
- 整机高 CPU 进程、热点线程和重点后台活动。
- ThermalService、冷却设备、cpuset、进程调度状态和 ADPF。
- 采集器日志、检查点、ADB 断线和恢复状态。
...
Mobile Power Profiler 和 BTR2 是两个独立进程：

```text
Android/HarmonyOS 手机 <-- Wi-Fi ADB --> Mobile Power Profiler --> 原始数据和报告
      ^
      |
机械臂/相机
      |
...
```text
power-runs\archives\运行名-evidence-时间戳.zip
```
...
不会删除 `power-runs`、`build` 或源码目录。

### 10.5 验证新包

```powershell
.\dist\mobile-power-profiler-portable\profiler.cmd --help
.\dist\mobile-power-profiler-portable\start-ui.bat --demo
```

如果需要包内 ADB，再检查：

```powershell
Test-Path .\dist\mobile-power-profiler-portable\platform-tools\adb.exe
```
...
- ThermalService 传感器、严重级别、冷却设备、公开阈值，以及 cpuset、进程调度
  状态和 ADPF HintSession。
...
- **时间线 / Flow**：功率、前台应用、测试项、系统活动、热状态和 ADPF 六类泳道。
- **测试项**：适合一小时多应用流程，逐项查看功率、CPU/GPU、温度和后台干扰。
- **应用**：按前台时间分配的整机实测能量。
- **CPU / GPU**：系统负载、频率和可用 GPU 证据。
- **系统活动**：进程、线程、GC、kworker、内核和 DEX/更新活动。
- **热控与调度**：温度严重级别、冷却设备、cpuset 和 ADPF。
...
# 采集一小时
mobile-power-profiler record --device SERIAL --duration 3720 `
  --session-mode --require-unplugged `
  --start-context desktop --start-note "BTR2 starts later" `
  --output power-runs\round-001

# 导入 BTR2 日志
mobile-power-profiler import-log RUN_DIR LOG_FILE `
...
# 恢复中断运行
mobile-power-profiler recover RUN_DIR
...
全局 `--adb PATH` 必须放在子命令前：

```powershell
mobile-power-profiler --adb D:\platform-tools\adb.exe probe --device SERIAL
```
...
- GPU 能力依赖 OEM 是否公开频率、busy/load 节点；不可读时只报告可获得的
...
- GC、kworker、DEX 等活动与功率的时间关联代表“可能贡献”和干扰证据，不等于
  已经完成物理电源轨级因果拆分。
- ADB 延迟、fuel gauge 更新周期和 OEM 传感器量化都会限制短时峰值精度。一小时级
...
- 不影响现有 Android/HarmonyOS ADB 采集和便携包。
- 将 GPL-3.0-or-later 的可选依赖保持在单独运行时与进程边界。
- 为不提供 ADB 通道的原生 HarmonyOS 接入保留同一套标准化 JSON/JSONL sidecar 协议。
...
mobile-power-profiler --ios-python $iosPython record `
  ...
  --session-mode `
  --require-unplugged `
  --output power-runs\ios-btr2-round-001 `
