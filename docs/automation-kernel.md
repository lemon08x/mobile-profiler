...
`mobile_profiler.automation` 的公共契约仍独立于模型协议和 UI；当前产品层已经接入第一
个可选实现 `Uiautomator2Provider`。`AdbAgentController` 可在纯视觉、纯语义和视觉加语义
三种模式之间选择，HTTP API 与前端只传递 `automation_engine`，功耗采集生命周期不因此
改变。

目标不是把第三方项目作为黑盒塞进测试流程，而是把其中可复用的机制重新实现为小型、
可替换的内部模块：上层只依赖契约，ADB、uiautomator2、图像识别、模型和游戏专用算法
分别通过适配器注入。当前接入只覆盖语义观察和白名单动作，不代表场景图、Policy、
Verifier、Watcher 或 Skill 已全部产品化。
...
契约和核心采集继续保持 Python 标准库零依赖。`uiautomator2` 作为可选 extra 延迟导入，
卸载后仍可使用纯 ADB 视觉引擎；OpenCV、Airtest 等后续实现也必须保持可选。
...
|---|---|---|---|
| [uiautomator2](https://github.com/openatx/uiautomator2) | 常驻语义服务、元素中心点击、UTF-8 输入、服务失效恢复 | `Uiautomator2Provider`、`SemanticUiProvider` | 已实现 active-root 压缩 UI 树、前台 package、统一 revision、revision 绑定的 `tap_element` 和一次恢复；截图由外层 Agent 同轮采集。纯 ADB UI dump 回退与稳定等待仍待实现 |
...
- `SemanticUiProvider` 负责组合任务所需的观察通道；可选
  `Uiautomator2Provider` 已接入 Agent，`AdbUiDumpProvider` 仍是后续回退实现。
...
- `AdbAgentController` 只接入语义 `Observation` 与白名单动作；尚未执行
  `ScenarioDefinition`，也未接入统一 `ActionPolicy`、`ApprovalGate` 或最终 Verifier。
- `uiautomator2` 是显式可选依赖；选择纯视觉时不会导入或连接它。选择语义/混合模式时，
  它会在目标手机部署自身运行组件，这是该引擎的预期行为。
- HTTP API 和 UI 已提供三种引擎选择，但没有自动并行跑三份任务；横评使用相同模型、
  Prompt 协议和起始状态顺序执行，并比较步骤、耗时、Token 与证据完整性。
- 尚未实现纯 ADB UI dump 回退、统一稳定等待、确定性图片/状态断言或自动引擎路由。
- 不导入第三方游戏脚本、图片、OCR 模型或业务资源包。

下一阶段可在不改公共契约的前提下实现 `AdbUiDumpProvider`、安全的
`AdbDeviceGateway`、`ui_tree_stable` Settler 和最终 Verifier，再把基准结果用于按应用
自动选择视觉、语义或混合引擎。
