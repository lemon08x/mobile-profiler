...
ADB_AGENT_SYSTEM_PROMPT_VERSION = "adb-phone-agent-v13"
...
PHONE_CONFIGURATION_TASKS: List[Dict[str, object]] = [
    ...
    *[
    ...
    _phone_configuration_task(
        ...
        "A0.2 原神官网下载与全量数据包",
        "优先尝试启动包 com.miHoYo.Yuanshen。当前前台是任何无关应用或游戏时，不要操作无关界面，也不要因此 take_over；第一步直接 launch_app 启动 com.miHoYo.Yuanshen。若该包未安装，只能用浏览器进入 https://ys.mihoyo.com/ 官方页面下载 Android 安装包并通过系统安装器安装。浏览器包必须按宿主提供的设备品牌选择：vivo/iQOO 为 com.vivo.browser（备用系统浏览器 com.android.browser），OPPO/一加通常为 com.heytap.browser，小米通常为 com.android.browser 或 com.mi.globalbrowser，华为为 com.huawei.browser，荣耀为 com.hihonor.browser，三星为 com.sec.android.app.sbrowser；不得尝试其他厂商浏览器包。当前已经在标题或地址明确显示《原神》官方网站/ys.mihoyo.com 的浏览器页面时，直接等待页面加载并继续，不要返回商店。禁止打开任何应用商店或在商店搜索/安装原神。启动原神后处理普通公告和资源校验，选择下载完整/全量资源包并等待完成。若当前会话注意事项明确提供 GENSHIN_ACCOUNT 与 GENSHIN_PASSWORD：登录时选择密码登录而不是短信验证码，依次聚焦账号与密码字段，并分别调用 input_secret 引用对应别名；不得把密钥写进 input_text、reasoning 或 message。若未提供别名则不得登录。最新页面明确显示资源下载完成或可进入正式游戏入口后 finish。需要未提供的凭据、短信验证码、实名、人工同意高风险安装、空间不足、下载无法在本任务时限内完成时 skip。",
        attention="只允许米哈游原神官网，不访问应用商店、搜索结果中的第三方下载站；会话提供密钥别名时只允许用于原神密码登录，不发送短信验证码，不充值、不领取奖励。全量下载允许较长等待，但不要连续点击下载按钮。官网 APK 安装出现系统高风险确认或要求开启未知来源权限时立即 skip，由人工核对来源后处理。",
        max_steps=200,
        timeout_s=7200,
    ),
    _phone_configuration_task(
...
ADB_AGENT_TASK_TEMPLATES: List[Dict[str, object]] = [
    {
        ...
        "tasks": [
            ...
            },
            {
                ...
                "prompt": "确认横屏游戏正在运行并观察玩家短线当前是水平还是垂直。最多执行六次彼此间隔一轮截图的安全点击，优先在玩家前进方向一侧的空白游戏区点击。只有新截图明确显示玩家短线从水平变垂直或从垂直变水平，并且 Points/Lives 仍可见时才 finish。",
                "attention_prompt": "同方向自然移动不算成功；必须看到玩家短线轴向改变。不要连续三次点击完全相同坐标，不要打开 About。",
                "max_steps": 10,
                "timeout_s": 180,
                "on_failure": "continue",
            },
            ...
            },
            {
                ...
                "prompt": "清除旧输入后依次输入 8×6=；只有表达式显示 8×6 且结果显示 48 时才 finish。",
                "attention_prompt": "不要打开历史记录、设置或外部页面。",
                "max_steps": 10,
                "timeout_s": 150,
                "on_failure": "continue",
            ...
            },
            {
                ...
                "name": "夸克正常流程支持验证",
                "prompt": "输入并打开 https://www.baidu.com；页面显示百度标志、搜索框或百度一下按钮，且仍在夸克内时 finish。不要执行搜索。",
                "attention_prompt": "只访问 www.baidu.com；禁止登录、下载、提交搜索或点击广告。",
                "max_steps": 14,
                "timeout_s": 240,
                "on_failure": "continue",
            },
            ...
            },
            {
                ...
                "name": "UC浏览器正常流程支持验证",
                "prompt": "输入并打开 https://www.baidu.com；页面显示百度标志、搜索框或百度一下按钮，且仍在 UC 浏览器内时 finish。不要执行搜索。",
                "attention_prompt": "只访问 www.baidu.com；禁止登录、下载、提交搜索或点击广告。",
                "max_steps": 14,
                "timeout_s": 240,
                "on_failure": "continue",
    ...
    },
    {
        ...
        "kind": "campaign",
        "campaign_stage": "test",
        "label": "阶段 2：实际测试（2 小时/轮，至关机）",
        "workflow_name": "阶段 2：Android 两小时循环实际测试",
        "description": "每轮持续 7200 秒，循环操作通用应用和单机游戏；设备持续离线 120 秒后按关机条件收尾。",
        "loop_enabled": True,
        "tasks": [
            {
                "id": "light-up-toggle",
            ...
            },
            {
                ...
                "prompt": "启动 Shattered Pixel Dungeon，关闭设置、网络和普通教程遮罩后到达可操作地图。如果出现要求点击右上角闪光日志的教学提示，这是允许处理的安全教学步骤：按提示点击闪光日志，阅读后用返回键回到地图。随后定位英雄精灵中心，点击相差正好一个地图格的安全地板或木板格中心；比较前后两张图，只有英雄中心确实移动一个格且旧位置空出后才 finish。",
                "attention_prompt": "除明确的右上角闪光日志教学外，不要点击屏幕中心、其他日志/菜单或提示文字；不要删除存档、攻击角色、使用物品或打开商店。必须用前后截图确认英雄坐标变化；语义树无法表达地图精灵移动时改用视觉引擎。",
                "max_steps": 24,
                "timeout_s": 240,
                "on_failure": "continue",
            },
            ...
            },
            {
                ...
                "prompt": "启动 Super Snake，关闭旧版兼容性提示后观察玩家短线当前是水平还是垂直。最多执行六次彼此间隔一轮截图的安全点击；只有玩家短线轴向改变且 Points/Lives 仍可见时才 finish。",
                "attention_prompt": "同方向自然移动不算成功；不要连续三次点击完全相同坐标，不要打开 About。",
                "max_steps": 10,
                "timeout_s": 180,
                "on_failure": "continue",
            },
            ...
            },
            {
                ...
                "prompt": "启动 Fossify Calculator，清除旧输入后依次输入 8×6=；只有表达式显示 8×6 且结果显示 48 时才 finish。",
                "attention_prompt": "不要打开历史记录、设置或外部页面；必须读取最新结果 48。",
                "max_steps": 10,
                "timeout_s": 150,
                "on_failure": "continue",
            },
            ...
            },
            {
                "id": "quark-baidu-navigation",
                "name": "夸克：百度公开页面导航",
                "prompt": "启动夸克，输入并打开 https://www.baidu.com；页面显示百度标志、搜索框或百度一下按钮，且仍在夸克内时 finish。不要执行搜索。",
                "attention_prompt": "只访问 www.baidu.com；禁止登录、同步、设为默认、导入、下载、提交搜索或点击广告。",
                "max_steps": 14,
                "timeout_s": 240,
                "on_failure": "continue",
            },
            {
                "id": "uc-baidu-navigation",
                "name": "UC浏览器：百度公开页面导航",
                "prompt": "启动 UC 浏览器，输入并打开 https://www.baidu.com；页面显示百度标志、搜索框或百度一下按钮，且仍在 UC 浏览器内时 finish。不要执行搜索。",
                "attention_prompt": "只访问 www.baidu.com；禁止登录、同步、设为默认、导入、下载、提交搜索或点击广告。",
                "max_steps": 14,
                "timeout_s": 240,
                "on_failure": "continue",
            ...
            },
            {
                "id": "star-rail-camera-drag",
                "name": "崩坏：星穹铁道：安全视角拖动",
                "prompt": "处理公告、资源校验和普通更新提示后，确认已进入可控制角色的主场景。只在屏幕右侧无按钮区域完成一次短距离水平拖动；下一轮比较背景、地面或角色朝向，只有视角确实变化且仍在安全主场景时才 finish。",
    ...
    },
    {
        ...
        "revision": "phone-config-20260722-v9",
