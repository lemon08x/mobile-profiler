...
class UiServerTests(unittest.TestCase):
    ...
    def test_demo_server_serves_assets_and_state(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn('id="agent-config-tab-campaign"', html)
            self.assertIn('id="agent-config-tab-software"', html)
            self.assertIn('data-agent-config-tab="campaign"', html)
            self.assertIn('data-agent-config-view="campaign"', html)
            ...
            self.assertIn('data-agent-prompt-tab="workflow"', html)
            self.assertIn('data-agent-prompt-tab="task"', html)
            self.assertIn('data-agent-prompt-tab="system"', html)
            self.assertIn('data-agent-prompt-view="workflow"', html)
            ...
            workflow_view = html[
                ...
                html.index('id="agent-config-view-campaign"')
            ]
            campaign_view = html[
                html.index('id="agent-config-view-campaign"'):
            ...
            self.assertIn('class="agent-launch-workspace"', workflow_view)
            self.assertIn('id="agent-launch-observation-title"', workflow_view)
            self.assertIn('id="agent-screen-image"', workflow_view)
            self.assertIn('id="agent-screen-meta"', workflow_view)
            self.assertIn("最新 ADB 截图", workflow_view)
            ...
            self.assertIn('class="agent-run-console"', workflow_view)
            self.assertIn('data-agent-run-tab="decision"', workflow_view)
            self.assertIn('data-agent-run-tab="results"', workflow_view)
            self.assertIn('data-agent-run-tab="logs"', workflow_view)
            self.assertIn('id="agent-reasoning-output"', workflow_view)
            self.assertIn('id="agent-task-results"', workflow_view)
            self.assertIn('id="agent-log-output"', workflow_view)
            ...
            self.assertNotIn('class="panel agent-screen-panel"', html)
            self.assertEqual(html.count('id="agent-screen-image"'), 1)
            self.assertEqual(html.count('id="agent-screen-meta"'), 1)
            self.assertIn("两阶段配置", campaign_view)
            self.assertIn('id="agent-campaign-config-source-path"', campaign_view)
            self.assertIn('data-agent-campaign-stage="prepare"', campaign_view)
            self.assertIn('data-agent-campaign-stage="test"', campaign_view)
            self.assertIn('id="agent-campaign-stage-panel"', campaign_view)
            self.assertIn("执行链路", campaign_view)
            self.assertIn("应用与游戏", software_view)
            self.assertIn('class="agent-module-layout agent-software-layout"', software_view)
            self.assertIn('data-agent-software-category="overview"', software_view)
            ...
            self.assertIn("从左侧选择分类查看具体软件", software_view)
            self.assertIn('id="agent-task-list"', prompt_view)
            self.assertIn('id="agent-workflow-name-input"', prompt_view)
            self.assertIn('class="agent-task-orchestration"', prompt_view)
            self.assertIn('id="agent-task-filter-input"', prompt_view)
            self.assertIn('id="agent-task-detail"', prompt_view)
            self.assertIn('id="agent-open-task-prompt-button"', prompt_view)
            self.assertIn("执行顺序与限制", prompt_view)
            self.assertIn('class="agent-module-layout agent-prompt-layout"', prompt_view)
            self.assertIn("任务编排", prompt_view)
            self.assertIn("任务 Prompt", prompt_view)
            ...
            self.assertIn("function agentPreparationConfigMarkup", javascript)
            self.assertIn("function agentTestConfigMarkup", javascript)
            self.assertIn("function agentCampaignDirectoryMarkup", javascript)
            self.assertIn("function setAgentRunConsoleSection", javascript)
            self.assertIn("动作下限", javascript)
            self.assertIn("动作上限", javascript)
            self.assertIn('phase: "初始化"', javascript)
            self.assertIn('phase: "主任务"', javascript)
            ...
            self.assertIn("function renderAgentWorkflowTaskEditor", javascript)
            self.assertIn("function selectAgentWorkflowTask", javascript)
            self.assertIn("function filterAgentTaskSequence", javascript)
            self.assertIn('const agentPromptTabs = ["workflow", "task", "system"]', javascript)
            ...
            self.assertIn("campaignAwareAgentTemplates", javascript)
            self.assertIn("runtime_task_overrides", javascript)
            self.assertIn("runtime_system_prompt_override", javascript)
            self.assertIn("payload.repeat_workflows", javascript)
            self.assertIn("payload.max_rounds = 1", javascript)
            self.assertIn("单轮内循环 workflow", html)
            self.assertIn('setAgentPromptTab("system")', javascript)
            self.assertIn('software: { title: "应用与游戏"', javascript)
            self.assertIn('campaign: { title: "阶段配置"', javascript)
            ...
            self.assertIn("function normalizeAgentActionLimits", javascript)
            self.assertIn("maximum_per_signature", javascript)
            self.assertIn('data-agent-task-field="action_limits"', javascript)
            self.assertIn("action_limits:", javascript)
            ...
            self.assertNotIn("height: calc(100vh - 188px)", css)
            self.assertIn(".agent-config-panel {\n  position: relative;", css)
            self.assertNotIn(".agent-config-panel {\n  position: sticky;", css)
            self.assertIn("width: min(100%, 300px);", css)
            self.assertIn(".agent-launch-workspace", css)
            self.assertIn(".agent-launch-observation", css)
            self.assertIn('.view[data-panel="agent"]', css)
            self.assertIn(".agent-software-summary", css)
            self.assertIn(".agent-campaign-stage-tabs", css)
            self.assertIn(".agent-campaign-flow", css)
            self.assertIn(".agent-campaign-config-layout", css)
            self.assertIn(".agent-campaign-warning-list", css)
            self.assertIn(".agent-software-categories", css)
            self.assertIn(".agent-module-directory", css)
            self.assertIn(".agent-run-console", css)
            self.assertIn(".agent-prompt-layout", css)
            self.assertIn(".agent-prompt-directory", css)
            self.assertIn(".agent-task-orchestration", css)
            self.assertIn(".agent-task-detail", css)
