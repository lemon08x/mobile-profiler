...
崩铁与终末地最初只有目录或运行时适配代码，并未跑通完整真机流程。此前目录层使用
...
可预检和可启动集合；字段为 false、缺失或快照异常一律禁止。2026-07-29，崩铁
StarRailCopilot 的完整自适应补丁在 2800×1260 真机完成世界 8 自然结算，并完成 daily
首轮与幂等复跑，因此与 MAA 一同满足单参考设备的端到端门禁；未安装完整补丁的 SRC
checkout 仍 fail-closed。
终末地继续可见仅用于独立重构，不代表可运行。
